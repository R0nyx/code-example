...
static void philosopher_1( void *pvParameters )
{
	...
	for( EVER )
	{
		...
		xSemaphoreTake(chopStick_1_Mutex,portMAX_DELAY);
		    ...
		    vTaskDelay( pause );
			//If we dont get it after while, we get pdFail = thus we timeout. 
		 if ( xSemaphoreTake(chopStick_2_Mutex, pause ) == pdFAIL ) //If statement is true if there was a blocked task meaning another tasks is using chopstick2 mutex.
		    {
			  xSemaphoreGive(chopStick_1_Mutex);
			    xSemaphoreTake(screenMutex,portMAX_DELAY);
			  	...
			  	xSemaphoreGive(screenMutex);
			  vTaskDelay( pause );
			  continue; //Try the for loop again.
			  //Skips the rest
		    }
