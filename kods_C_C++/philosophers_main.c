...
static xQueueHandle rightChopstickQueue; //Create queue handle.

static void philosopher_1(void *vptr ); //Function prototypes for all philosophers.
static void philosopher_2(void *vptr );
static void philosopher_3(void *vptr );
static void philosopher_4(void *vptr );

static xSemaphoreHandle screenMutex; //Create handle for screenmutex.
static xSemaphoreHandle chopStick_1_Mutex; //Create handle for chopstick mutexes.
...
int main(void)
{
 ...
 // freeRtos stuff end
      ...
      screenMutex = xSemaphoreCreateMutex(); //Create screenmutexes.
    ...
    //  rightChopstickQueue =  xQueueCreate(1, sizeof(struct Req));
	  xTaskCreate( philosopher_1,
				...
				NULL,
				1,
				NULL ); //Create task for philosopher1. First argument being a pointer to the function. Second argument being a descriptive name for the task usually used for debugging. Third argument being the number of words to allocate for use as
...
static void philosopher_1( void *pvParameters ) //Function for philosopher1.
{
	//portTickType xNextWakeTime;

	for( EVER ) //Infinite loop
	{
		xSemaphoreTake(chopStick_1_Mutex,portMAX_DELAY); //Use chopstick1 mutex with portmax_delay as timeout interval causing chopstick1 to be owned.
		    xSemaphoreTake(screenMutex,portMAX_DELAY); //Use screenmutex with portmax_delay as timeout interval while printing to the LCD.
		      LCD_DisplayStringLine(LINE(3),(uint8_t*)"ph1 has ch1" ); //Print to the LCD that philosopher1 has chopstick1.
		    xSemaphoreGive(screenMutex); //Release the screenmutex.
		    vTaskDelay( pause ); //Delay the task by the amount of time specified by variable pause.

		  xSemaphoreTake(chopStick_2_Mutex,portMAX_DELAY);
		    xSemaphoreTake(screenMutex,portMAX_DELAY);
		      LCD_DisplayStringLine(LINE(3),(uint8_t*)"ph1 has ch2" ); //Print to the LCD that philosopher1 has chopstick2.
		   ...
		   // vTaskDelayUntil( &xNextWakeTime, pause );
		    ...
		    xSemaphoreTake(screenMutex,portMAX_DELAY);
		      LCD_DisplayStringLine(LINE(3),(uint8_t*)"ph1 eats    " ); //Print to LCD that philosopher1 is now eating.
		    xSemaphoreGive(screenMutex);
		   // vTaskDelayUntil( &xNextWakeTime, pause );
		    vTaskDelay( pause );

		  xSemaphoreGive(chopStick_2_Mutex); //Release chopstick2 mutex causing it to be signaled(available).
		     xSemaphoreTake(screenMutex,portMAX_DELAY);
		  		LCD_DisplayStringLine(LINE(3),(uint8_t*)"ph1 puts ch2" ); //Print that philosopher1 puts down chopstick2.
		  	 xSemaphoreGive(screenMutex);
		  //	vTaskDelayUntil( &xNextWakeTime, pause );
		  	vTaskDelay( pause );
		 ...
		 xSemaphoreGive(chopStick_1_Mutex);
		   xSemaphoreTake(screenMutex,portMAX_DELAY);
		  	  LCD_DisplayStringLine(LINE(3),(uint8_t*)"ph1 puts ch1" ); //Print that philosopher1 puts down chopstick1.
		   xSemaphoreGive(screenMutex);
		   vTaskDelay( pause );// vTaskDelayUntil( &xNextWakeTime, pause );

		 xSemaphoreTake(screenMutex,portMAX_DELAY);
		   	  LCD_DisplayStringLine(LINE(3),(uint8_t*)"ph1 thinks     " ); //Print that philosopher1 is thinking.
		 xSemaphoreGive(screenMutex);
		 vTaskDelay( pause );// vTaskDelayUntil( &xNextWakeTime, pause );
