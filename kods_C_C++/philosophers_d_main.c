...
int main(void)
{
 ...
 // freeRtos stuff end
      ...
      rightChopstickQueue =  xQueueCreate(3, sizeof(rightChopStickTicket)); //Create queue. First parameter being the maximum number of items the queue can hold. Second parameter is the size in bytes required to hold each item in the queue.

	  //Sending 3 tickets to the queue. 
      xQueueSend(rightChopstickQueue,(int*)&rightChopStickTicket,portMAX_DELAY);//Post an item on queue. The item is queued by copy not reference. Arguments: The handle to the queue, a pointer to the item that is to be placed on the queue,
																				//the maximum time the task should block waiting for space to become available on the queue, should it already be full.
      xQueueSend(rightChopstickQueue,(int*)&rightChopStickTicket,portMAX_DELAY);
      xQueueSend(rightChopstickQueue,(int*)&rightChopStickTicket,portMAX_DELAY);
...
{
	int rightChopStickTicket;

	for( EVER )
	{	//Philosopher gets a ticket for the queue 
		xQueueReceive(rightChopstickQueue,&rightChopStickTicket,portMAX_DELAY); //Call to the function that receives an item from the queue. arguments being: handle to the queue, pointer to the buffer holding a copy of the receive item,
																					  //the maximum amount of time the task should block waiting for an item should the queue be empty at the time of the call.
		xSemaphoreTake(chopStick_1_Mutex,portMAX_DELAY); //Lock the mutex chopstick1 while work is being done or until the timeout interval expires.
		    xSemaphoreTake(screenMutex,portMAX_DELAY); //Lock the mutex screen mutex while work is being done or until the timeout interval expires.
		      LCD_DisplayStringLine(LINE(3),(uint8_t*)"ph1 has ch1" ); //Print to the LCD that philosopher1 has chopstick1.
		    xSemaphoreGive(screenMutex); //Release screen mutex by changing the state to signaled.
		    vTaskDelay( pause ); //Delay task by value of pause.

		  xSemaphoreTake(chopStick_2_Mutex,portMAX_DELAY); //Lock the mutex chopstick2 while work is being done or until the timeout interval expires.
		    xSemaphoreTake(screenMutex,portMAX_DELAY); //Lock the mutex screen mutex while work is being done or until the timeout interval expires.
		      LCD_DisplayStringLine(LINE(3),(uint8_t*)"ph1 has ch2" ); //Print that philosopher2 picked up chopstick2 to the LCD.
		  ...
		  xSemaphoreGive(chopStick_2_Mutex); //Release mutex chopstick2 by changing the state to signaled because philosopher1 is done using it for eating.
		     xSemaphoreTake(screenMutex,portMAX_DELAY);
		 ...
		 xSemaphoreGive(chopStick_1_Mutex); //Release mutex chopstick1 by changing the state to signaled because philosopher1 is done using it for eating.
		 xQueueSend(rightChopstickQueue,	&rightChopStickTicket,	portMAX_DELAY); //Post an item on the queue because philosopher1 is done using his chopsticks.
		   xSemaphoreTake(screenMutex,portMAX_DELAY);
...
static void philosopher_2( void *pvParameters )
{
	...
	for( EVER )
	{
		xQueueReceive(rightChopstickQueue,(int*)&rightChopStickTicket,portMAX_DELAY);
